...
if [ $stage -le 6 ]; then
  ...
  steps/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 1 \
    --online-ivector-dir exp/nnet2_online/ivectors_test \
    exp/tri3/graph data/test $dir/decode  &

  steps/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 1 \
    --online-ivector-dir exp/nnet2_online/ivectors_test \
    exp/tri3/graph_ug data/test $dir/decode_ug || exit 1;
...
if [ $stage -le 8 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 1 \
    exp/tri3/graph data/test ${dir}_online/decode &
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 1 \
    exp/tri3/graph_ug data/test ${dir}_online/decode_ug || exit 1;
  wait
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 1 \
    --per-utt true \
    exp/tri3/graph data/test ${dir}_online/decode_per_utt &
  steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj 1 \
    --per-utt true \
    exp/tri3/graph_ug data/test ${dir}_online/decode_ug_per_utt || exit 1;
  wait
fi
